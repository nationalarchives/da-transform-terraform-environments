...
module "validate_bagit" {
  ...
  source = "github.com/nationalarchives/da-transform-terraform-modules?ref=prod//step_functions/validate_bagit"
  env = var.environment_name
  prefix = var.prefix
  account_id = data.aws_caller_identity.aws.account_id
...
module "dri_preingest_sip_generation" {
  source = "github.com/nationalarchives/da-transform-terraform-modules?ref=prod//step_functions/dri_preingest_sip_generation"
  env = var.environment_name
  prefix = var.prefix
  account_id = data.aws_caller_identity.aws.account_id
  common_tre_slack_alerts_topic_arn = module.common.common_tre_slack_alerts_topic_arn
