module "pipeline_step_function" {
  source = "github.com/nationalarchives/da-transform-terraform-modules?ref=int//step_function"
...
module "tdr_sqs_in_queue" {
  source = "github.com/nationalarchives/da-transform-terraform-modules?ref=int//sqs"
  env = var.environment_name
  prefix = var.prefix
...
module "common" {
  source = "github.com/nationalarchives/da-transform-terraform-modules?ref=int//common"
  env    = var.environment_name
  prefix = var.prefix
  account_id = data.aws_caller_identity.aws.account_id
  image_versions = var.image_versions
...
module "receive_and_process_bag" {
  source = "github.com/nationalarchives/da-transform-terraform-modules?ref=int//step_functions/receive_and_process_bag"
  env = var.environment_name
  prefix = var.prefix
  account_id = data.aws_caller_identity.aws.account_id
  ...
  tdr_sqs_retry_url = var.tdr_sqs_queue_endpoint
  tdr_sqs_retry_arn = var.tdr_sqs_queue_arn
}
